...
export class ProductGroupDetailComponent implements OnInit {
  ...
  public submit() {
    ...
    this.http.put<ProductGroup>(`${this.config.productGroupDetailApi.endpoint}/${pg.id}`, pg, options)
    .pipe(catchError(err=>{
  ...
  constructor(private http:HttpClient, private formBld:FormBuilder, private act: ActivatedRoute, private authSvc: OAuthService, @Inject(APP_CONFIG)private config:IAppConfig) { 
  ...
  ngOnInit(): void 
  {
    ...
    this.act.params.pipe(
      ...
      switchMap(param=> this.http.get<ProductGroup>(`${this.config.productGroupDetailApi.endpoint}/${+param["id"]}`, options))
    ).subscribe(pg=>{
